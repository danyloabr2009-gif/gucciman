    ...
    async def press_button(self, client: TelegramClient, chat_id: int, message_id: int, button_info: ButtonInfo) -> bool:
        ...
        try:
            if button_info.type == 'callback' and button_info.data:
                ...
                # Handle URL buttons
                pass  # URL buttons need special handling
            else:
                return False
            
            self.logger.success(f"✅ Кнопка '{button_info.text}' нажата успешно")
            return True
            
        except FloodWaitError as e:
            wait_time = e.seconds
            self.logger.warning(f"⏰ FloodWait: жду {wait_time} секунд...")
            await asyncio.sleep(wait_time)
            return False
            
        except Exception as e:
            self.logger.error(f"❌ Ошибка нажатия кнопки: {e}")
            return False
            ...
            if button_info.type == 'url' and button_info.url:
                try:
                    ...
                    if 't.me/' in button_info.url:
                        ...
                        subscribed_count += 1
                        self.logger.success(f"📡 Успешная подписка на @{channel_username}")
                    ...
                    self.logger.info(f"ℹ️ Уже подписан на @{channel_username}")
                    subscribed_count += 1
                    
                except (ChannelPrivateError, ChatAdminRequiredError) as e:
                    self.logger.warning(f"⚠️ Не удалось подписаться на @{channel_username}: {e}")
                    
                except Exception as e:
                    self.logger.error(f"❌ Ошибка подписки на @{channel_username}: {e}")
